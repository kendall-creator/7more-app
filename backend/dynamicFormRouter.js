...
if (!admin.apps.length) {
  try {
    ...
    if (process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
      ...
      // Fallback: Initialize without auth (read-only access to public data)
      admin.initializeApp({
        databaseURL: "https://sevenmore-app-5a969-default-rtdb.firebaseio.com"
      });
    }
    console.log("✅ Firebase Admin initialized for dynamic forms");
  } catch (error) {
...
router.get('/forms/participant-intake', async (req, res) => {
  try {
    // Instead of fetching from Firebase server-side, we'll let the client fetch it
    // This avoids Firebase Admin SDK issues and works better with client permissions
    const html = generateFormHTML();
    res.setHeader('Content-Type', 'text/html');
    res.send(html);
  } catch (error) {
    console.error('Error rendering participant intake form:', error);
...
// Generate HTML from form configuration
...
<head>
    ...
    <title>${formConfig.title || 'Participant Intake Form'}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1>${formConfig.title || 'Participant Intake Form'}</h1>
        <p class="description">${formConfig.description || 'Please fill out this form to apply for our program'}</p>
        ...
        <form id="intakeForm">
            ${enabledFields.map(field => generateFieldHTML(field)).join('')}
    ...
    <script type="module">
        ...
        // Handle conditional fields (e.g., "Other" option)
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Handle "Other" field for releasedFrom
...
function generateFieldHTML(field) {
  ...
  switch (field.type) {
    ...
    case 'select':
      ...
      // Add conditional "Other" field if needed
      if (field.options && field.options.includes('Other')) {
        html += `
          <div id="${field.id}_other_field" class="conditional-field" style="display: none;">
            <label for="${field.id}_other">Please specify:</label>
            <input type="text" id="${field.id}_other" name="${field.id}_other" placeholder="Enter details">
          </div>
        `;
      }
      break;
